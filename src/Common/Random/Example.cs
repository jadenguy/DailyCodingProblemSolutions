...
public class Example
{
    ...
    public static async Task Main()
    {
        ...
        await ex.Execute();
        System.Console.WriteLine(x.ElapsedMilliseconds);
            ...
            tasks.Add(Task.Run(() =>
                {
                    ...
                    double result = 0.0;
                    Random randThread;
                    lock (randLock) { randThread = new Random(rand.Next()); }
                    for (int n = 0; n < 20000000; n++)
                    {
                        // Make sure there's no corruption of Random.
                        token.ThrowIfCancellationRequested();
                        lock (randLock) { result = randThread.NextDouble(); }

                        // Check for corruption of Random instance.
                        if ((result == previous) && result == 0) { source.Cancel(); }
